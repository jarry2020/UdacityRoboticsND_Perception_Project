...
# Exercise-3 TODOs:
    ...
    for index, pts_list in enumerate(cluster_indices):
        ...
        chists = compute_color_histograms(ros_cluster, 128, True)
        normals = get_normals(ros_cluster)
        nhists = compute_normal_histograms(normals,128)
...
def pr2_mover(object_list):
    ...
    for obj in object_list_param:
        ...
        for detected_object in object_list:
            if detected_object.label == object_name.data:
                points_array = ros_to_pcl(detected_object.cloud).to_array()
                pose_m = np.mean(points_array, axis=0)[:3]
                pick_pose.position.x = np.asscalar(pose_m[0])
                pick_pose.position.y = np.asscalar(pose_m[1])
                pick_pose.position.z = np.asscalar(pose_m[2])
                break
    ...
    send_to_yaml('output_%i.yaml' % test_scene_num.data, dict_list)
...
if __name__ == '__main__':
    ...
    # Call them object_markers_cloud_objectspub and detected_objects_pub
